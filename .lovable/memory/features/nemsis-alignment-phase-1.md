...
- Analyzed & no migration needed: VitalsCard, MedicationsCard, ProceduresCard, ConditionOnArrivalCard (LOC/skin), AssessmentCards, EquipmentCard, PatientInfoCard sex.
- Still to audit against billing readers: IVAccessCard, IsolationPrecautionsCard, BehavioralHealthCard, PatientInfoCard (race/ethnicity/gender), SignaturesCard, NarrativeCard (disposition).
- **HAZARD — StretcherMobilityCard.** `edi-837p-generator.ts` (ambulance modifier `09`) and `claim-readiness.ts` (bed-confinement inference) both do `String(stretcher_placement).toLowerCase() !== "ambulatory" && !== "none"` checks against a column populated from `STRETCHER_OPTIONS` (Draw Sheet, Manual Lift, Backboard, etc.). Neither "ambulatory" nor "none" exists in the option list, so the branch fires whenever the field is populated — the logic works by accident today. Any migration that swaps this to a NEMSIS-coded value MUST preserve the invariant "stored value is non-empty AND does not lowercase to `ambulatory`/`none`" or add explicit code translation before those comparisons. Do NOT migrate without a claim-parity test that exercises stretcher/ambulatory/none paths.
...
**Round-3 audit results (this session)**
- **AssessmentCards (chief_complaint, primary_impression)** — edi-837p-generator NTE segments write these display strings verbatim and compare to `"Other"` to swap in the free-text override. The stored column IS the 837P payload; do not touch it. NEMSIS eSituation.09/.11 want ICD-10 codes, and those are already collected separately in `claim_records.icd10_codes` — the Phase 5 exporter pulls the coded value from that array and pairs it with the display string. No dual-write column needed.
- **EquipmentCard (oxygen_delivery_method)** — appears only in `pcr-narrative.ts` (equipment phrase). No billing string comparison. `E_OXYGEN_DELIVERY` already registered; no card change.
- **PatientInfoCard sex** — `edi-837p-generator.ts` normalizes to M/F/U via `dmgSexCode()`. The column already stores a code-shaped value. Registered `E_PATIENT_SEX` with both NEMSIS codes (9906001/3/5/7) and M/F/U aliases so `findByCode()` resolves whichever shape sits in the row. Phase 5 XML export uses the NEMSIS code; 837P keeps M/F/U. No dual-write column needed.