...
export default function PendingCompaniesAdmin() {
  ...
  const handleApprove = async (companyId: string) => {
    ...
    try {
      const { error } = await supabase.functions.invoke("manage-company", { body: { companyId, action: "approve" } });
  ...
  const handleReject = async (companyId: string) => {
    ...
    setActionLoading(companyId);
    try {
      const { error } = await supabase.functions.invoke("manage-company", { body: { companyId, action: "reject", reason } });
      if (error) throw error;
