...
interface Props {
  ...
  crewMember3?: { id: string; name: string; cert: string } | null;
  onSelect: (medic: { id: string; name: string; cert: string }) => void;
}

export function MedicSelector({ crewMember1, crewMember2, crewMember3, onSelect }: Props) {
  const members = [crewMember1, crewMember2, crewMember3].filter(Boolean) as { id: string; name: string; cert: string }[];
