  ...
  "/scheduling": {
    ...
    content: {
      does: [
        ...
        "Daily Ops Snapshot bar: at the top of each day view, shows active trucks, total runs, unassigned count, avg runs/truck, empty trucks, overloaded trucks (>8 runs), DOWN trucks, and trucks with no crew — all at a glance.",
        "Overloaded means a truck has more than 8 runs — it's a density warning, not a hard block. Adjust the threshold in code if your fleet has different norms.",
        "Run Pool: lists all generated/created runs for that day that are not yet assigned to a truck.",
        "Auto-Fill: generates A and B legs from patient recurrence profiles (Dialysis/Outpatient) for the selected date — skips patients who are inactive, in hospital, on vacation, or outside their recurrence window. Will not create duplicate legs if already generated.",
        "Lets you manually create A-legs (pickup to facility) and B-legs (return trip) for ad-hoc runs (discharge, hospital, private pay).",
        "Exception editing: click the pencil icon on any run to change pickup location, time, or notes for THAT date only — the recurring series is not affected. A branch icon marks exception runs.",
        "Lets you assign runs from the Run Pool into a specific truck using 'Add Leg from Run Pool'.",
        "Lets you drag-and-drop legs within a truck to reorder them.",
      ...
      tips: [
        ...
        "Ad-hoc runs (discharge, hospital, private pay) are created manually — they don't auto-fill.",
        "If a truck goes DOWN after runs are assigned: open that truck's card, use the trash icon on each leg to move it back to the Run Pool, then assign to another truck.",
        "The snapshot bar resets each day — it only reflects the selected day's data.",
      ],
    },
  },
  ...
  "/crew/:token": {
    ...
    content: {
      ...
      tips: [
        ...
        "Exception overrides (different pickup location or time for today only) will show automatically.",
      ],
    },
  },
};
