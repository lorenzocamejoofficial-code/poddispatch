...
BEGIN
  ...
  BEGIN
    ...
    -- Honest status: the database cannot evaluate the ~25 hard-block readiness
    -- rules that live in src/lib/claim-readiness.ts, and duplicating them here
    -- would create a second, divergent source of truth. So creation is
    -- pessimistic: every new claim starts in needs_review, and only the
    -- TypeScript readiness gate (buildClaimFromTrip / refreshExistingClaims)
    -- may promote it to ready_to_bill once it is provably clean.
    ...
    -- Do NOT wipe the trip's readiness signal here. trip_records.blockers,
    -- billing_blocked_reason and claim_ready are the Trip Queue's single source
    -- of truth; clearing them made a blocked trip look clean in two places.
