...
BEGIN
  ...
  BEGIN
    ...
    v_is_emergency := lower(coalesce(NEW.trip_type, NEW.pcr_type, '')) = 'emergency';
    v_is_unscheduled := coalesce(NEW.is_unscheduled, false);
    ...
    -- RED: PCS missing/expired — skip for emergency and same-day unscheduled runs
    IF NEW.patient_id IS NOT NULL AND NOT v_is_emergency AND NOT v_is_unscheduled THEN
