...
SET claim_ready = true,
    billing_blocked_reason = NULL,
    blockers = '{}'::text[],
    status = CASE WHEN tr.status = 'completed'::public.trip_status
                  THEN 'ready_for_billing'::public.trip_status
                  ELSE tr.status END,
    updated_at = now()
FROM public.claim_records cr
WHERE cr.trip_id = tr.id
  AND tr.claim_ready = false
  AND cr.status::text IN ('ready_to_bill','submitted','paid');
